...
import com.github.ros.roxanne_sa.ai.framework.domain.component.sv.PrimitiveStateVariable;
import com.github.ros.roxanne_sa.ai.framework.domain.component.sv.StateVariableValue;
import com.github.ros.roxanne_sa.ai.framework.domain.component.sv.ValuePath;
...
import com.github.ros.roxanne_sa.ai.framework.microkernel.resolver.timeline.behavior.planning.Gap;
import com.github.ros.roxanne_sa.ai.framework.microkernel.resolver.timeline.behavior.planning.GapCompletion;
import com.github.ros.roxanne_sa.ai.framework.microkernel.resolver.timeline.scheduling.BinaryDecisionConflict;
...
public class StateVariableComponentTestCase 
{
	...
	public void detectStateVariablePeaksTest() {
		...
		try 
		{
			...
			for (Flaw f : flaws) {
				...
				Assert.assertNotNull(f.getSolutions());
				Assert.assertFalse(f.getSolutions().isEmpty());
	...
	public void detectAndSolveStateVariablePeaksTest() {
		...
		try 
		{
			...
			Assert.assertNotNull(flaws.get(0));
	...
	public void detectAndSolveGapFlaws() {
		...
		try 
		{
			...
			Assert.assertTrue(this.psv.getPendingDecisions().size() >= 1);
			System.out.println("Pending decisions");
			System.out.println(this.psv.getPendingDecisions());
			
			// activate pending decisions with related relations
	...
	public void commitAndRollbackStateVariableGapFlaws() {
		...
		try 
		{
			...
			Assert.assertFalse(this.psv.getActiveDecisions().isEmpty());
			Assert.assertTrue(this.psv.getActiveDecisions().size() == 2);
			Assert.assertTrue(this.psv.getPendingDecisions().size() == 0);
			Assert.assertTrue(this.psv.getSilentDecisions().size() == 2);
			
			System.out.println("Active Decisions:\n" + this.psv.getActiveDecisions());
			Assert.assertNotNull(this.psv.getPendingDecisions());
			Assert.assertTrue(this.psv.getPendingDecisions().isEmpty());
			System.out.println("Pending decisions:\n" + this.psv.getPendingDecisions());
			Assert.assertFalse(this.psv.getSilentDecisions().isEmpty());
			System.out.println("Silent decisions:\n" + this.psv.getSilentDecisions());
	...
	public void commitAndRollbackStateVariableSchedulingFlaws() {
		...
		try 
		{
			...
			// get peak
			BinaryDecisionConflict peak = (BinaryDecisionConflict) flaws.get(0);
			// check solutions
